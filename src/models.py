...
#from utils.models_util import cache_initialization
...
class DiT(nn.Module):
    ...
    def cache_initialization(self, cache_folder, cache_threshold=1.0):
        self.cache_threshold = cache_threshold
        self.cache_attn = []
        self.cache_mlp = []

        self.cache_attn = torch.load(osp.join(cache_folder,'min_attn_dict_'+cache_threshold+'.pth'))
        self.cache_mlp = torch.load(osp.join(cache_folder,'min_mlp_dict_'+cache_threshold+'.pth'))
    ...
    def forward(self, x, t, current_step, cache, y):
        """
        Forward pass of DiT.
        ...
        for layeridx, block in enumerate(self.blocks):
            x = block(x, c, layeridx, current_step, cache, self.cache_attn, self.cache_mlp, self.num_steps, self.cache_threshold)                      # (N, T, D)
    ...
    def forward_with_cfg(self, x, t, current_step, cache, y, cfg_scale):
        """
        ...
        model_out = self.forward(combined, t, current_step, cache, y)
